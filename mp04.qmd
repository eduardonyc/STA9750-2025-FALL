...
To assess the stability of United States labor market data, it is essential to first acquire the official historical record of U.S. employment. Unlike static file downloads, the Bureau of Labor Statistics (BLS) serves this data dynamically rather than via static files. 
...
Having acquired absolute employment levels (Task 1) and revision magnitudes (Task 2), this task focuses on data integration. By joining these datasets on their date index, we construct a comprehensive analytical dataframe. This approach enables the calculation of critical metrics, specifically the magnitude of revisions relative to the total labor force.
...
bind_rows(top_positive, top_negative) |>
  ...
  scale_x_continuous(
    ...
    expand = expansion(mult = c(0.15, 0.15))  # Add 15% padding on both sides
  ) +
  labs(
...
year_freq <- ces_combined |>
  ...
  arrange(year)  # Ascending order from 1979
...
datatable(
  ...
  options = list(
    pageLength = 5,
    lengthMenu = c(5, 15, 25, 35, 47),  # 47 is total years
    dom = 'Blfrtip',
    buttons = c('copy', 'csv', 'excel'),
    ordering = TRUE,
    order = list(list(0, 'asc')),  # Sort by year ascending
    columnDefs = list(
...
# Create clean table (no buttons, search, or pagination)
datatable(
  decade_freq,
  caption = tags$caption(
    style = 'caption-side: top; text-align: center; color: black; font-size: 18px; font-weight: bold;',
  ...
  options = list(
    ...
    ordering = FALSE,  # Disable sorting to keep chronological order
    columnDefs = list(
...
ggplot(decade_summary, aes(x = decade, y = pct_positive)) +
  ...
  scale_fill_manual(
    values = c("TRUE" = "#2E86AB", "FALSE" = "#A23B72"),
    labels = c("FALSE" = "Majority Negative", "TRUE" = "Majority Positive"),  # FIXED: Explicitly map labels
    name = "Bias Direction"
  ) +
  scale_y_continuous(
...
q3_visual_data <- ces_combined |>
  ...
  filter(revision_pct_of_final <= 100)  # This removes April 1979's 580%
...
# Calculate monthly statistics (self-contained)
...
***Note: There is an omission of two data points, March 2003 and April 2003, as the final estimate was 0, yielding an empty set for revision data. Please refer to Table 7 and type "2003" in search bar to view detailed results. Consequently, there will be 556 total observations examined in this section.***
...
absolute_stats <- ces_combined |>
  summarise(
    ...
    `Standard Deviation` = sd(revision, na.rm = TRUE)  # CHANGED: Use signed revision
  ) |>
...
ggplot(ces_combined, aes(x = revision_pct_of_level)) +
  ...
  scale_x_continuous(
    ...
    breaks = seq(0, 0.50, by = 0.10),  # CHANGED: Break every 0.10% instead of 0.05%
    minor_breaks = seq(0, 0.50, by = 0.05),  # Show minor gridlines at 0.05%
    limits = c(0, NA),
    expand = c(0, 0)
  ) +
  scale_y_continuous(
  ...
  theme(
    plot.title = element_text(face = "bold", size = 14),
    plot.subtitle = element_text(size = 11),
    axis.text.x = element_text(angle = 0, hjust = 0.5, size = 11),  # Horizontal, no rotation needed
...
# Recreate all tests independently (self-contained)
...
# REUSE the plot from Task 4, but update labels for Fact Check context
plot_test3 +
  labs(
...
# REUSE the plot from Task 4, but update labels
plot_test2 +
  labs(
...
# Display results - FIXED: only 6 columns
test2_result |>
  kable(
...
- How many times have are revisions this large or larger?
...
# Recalculate needed values (self-contained)
combined_may_june_2025 <- ces_combined |>
  filter(date %in% as.Date(c("2025-05-01", "2025-06-01"))) |>
  summarise(combined = sum(abs(revision), na.rm = TRUE)) |>
  pull(combined)
...
### PolitiFact Rating: Mostly True (REVISE BEFORE POSTING!!!)
...
percentile_exceptional <- percentile_rank_258 >= 98  # Top 2%
sd_exceptional <- abs(z_score_258) > 2.5  # Beyond 2.5 SD
...
### **PolitiFact Rating: `r politifact_rating`**

**Rating Decision Logic**:

```{r}
#| echo: false
cat("Criterion 1 - Percentile Test: ", percentile_rank_258, "th percentile\n")
cat("  → Exceptional if ≥98th percentile: ", percentile_exceptional, "\n\n")
cat("Criterion 2 - Standard Deviation Test: ", round(z_score_258, 2), " SD from mean\n")
cat("  → Exceptional if >2.5 SD: ", sd_exceptional, "\n\n")
cat("DECISION: ", rating_explanation, "\n")
cat("RATING: ", politifact_rating, "\n")
```
