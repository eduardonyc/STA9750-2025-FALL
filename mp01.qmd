...
# --- Standardize season labels in both tables ---
COUNTRY_ST <- COUNTRY_TOP_10 %>%
  filter(show_title == "Stranger Things") %>%
  mutate(
    # pull the first number seen in season_title; default to 1 if missing
    season_num = if_else(is.na(season_title) | season_title == "",
                         1L,
                         suppressWarnings(as.integer(str_extract(season_title, "\\d+")))),
    season_num = pmin(pmax(season_num, 1L), 4L),  # clamp to 1..4 just in case
...
GLOBAL_ST <- GLOBAL_TOP_10 %>%
  ...
  mutate(
    season_num = if_else(is.na(season_title) | season_title == "",
                         1L,
                         suppressWarnings(as.integer(str_extract(season_title, "\\d+")))),
    season_num = pmin(pmax(season_num, 1L), 4L),
    season_label = paste0("Stranger Things: S", season_num)
  )

# --- Countries reached & country-weeks per season (from COUNTRY TSV) ---
...
# --- Global weeks & hours per season (from GLOBAL TSV) ---
...
# --- Combine + order S1 -> S4 ---
ST_SEASON_IMPACT <- ST_COUNTRIES_BY_SEASON %>%
  left_join(ST_GLOBAL_BY_SEASON, by = c("season_num", "season_label")) %>%
  mutate(`Total Global Hours` = comma(`Total Global Hours`)) %>%
  arrange(season_num) %>%
  select(
    Season = season_label,
    `Countries Reached`,
...
datatable(
  ST_SEASON_IMPACT,
  caption = "Stranger Things – Multinational Appeal by Season (Ascending Order)",
  options = list(
