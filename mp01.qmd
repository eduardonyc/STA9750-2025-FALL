...
unique_title_seasons <- latin_table %>%
  ...
  summarise(
    `Global Hours Viewed` = first(`Global Hours Viewed`),  # already totals from global_metrics
    .groups = "drop"
  ) %>%
  # 2) Convert hours → "views" assuming ~1 hour per view (identical numerically)
  mutate(`Approx Global Views` = `Global Hours Viewed`) %>%
  arrange(desc(`Global Hours Viewed`))

# 3) Render table (searchable, 5 rows per page, with paging)
datatable(
  unique_title_seasons,
