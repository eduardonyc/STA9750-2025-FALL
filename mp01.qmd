...
# -----------------------------
# 1) Countries (LATAM, excl. Brazil) — 15 exactly
# -----------------------------
latam_15 <- c(
  "Colombia","El Salvador","Honduras","Nicaragua","Venezuela",
  "Guatemala","Ecuador","Paraguay","Panama","Costa Rica",
  "Peru","Bolivia","Argentina","Chile","Dominican Republic"
)

# -----------------------------
# 2) For each country: top (show, season) by cumulative weeks
# -----------------------------
latam_top_per_country <- COUNTRY_TOP_10 %>%
  filter(country_name %in% latam_15) %>%
  mutate(season_title = coalesce(season_title, "")) %>%   # make joins NA-safe
  group_by(country_name) %>%
  # Keep the single best title; break ties by greater cumulative weeks, then earlier week
  slice_max(cumulative_weeks_in_top_10, n = 1, with_ties = FALSE) %>%
  ungroup()

# -----------------------------
# 3) Global totals for those titles (sum of all weeks) + best global rank
# -----------------------------
global_metrics <- GLOBAL_TOP_10 %>%
  mutate(season_title = coalesce(season_title, "")) %>%
  group_by(show_title, season_title) %>%
  summarise(
    global_total_hours = sum(weekly_hours_viewed, na.rm = TRUE),
    global_best_rank   = suppressWarnings(min(weekly_rank, na.rm = TRUE)),
    .groups = "drop"
  )

# -----------------------------
# 4) Combine and format like the screenshot
# -----------------------------
latin_table <- latam_top_per_country %>%
  select(
    Country = country_name,
  ...
  left_join(global_metrics,
            by = c("Show Title" = "show_title", "Season Title" = "season_title")) %>%
  transmute(
    Country,
    `Show Title`,
    `Season Title` = ifelse(`Season Title` == "", "N/A", `Season Title`),
    `Cumulative Weeks in Top 10`,
    `Global Hours Viewed` = global_total_hours,
    `Global Best Rank`    = global_best_rank
  ) %>%
  arrange(desc(`Cumulative Weeks in Top 10`), Country)
...
datatable(
  latin_table,
  caption = "Top Latin American Countries’ Top Shows: Regional vs Global Performance",
  options = list(
...
# 1) Collapse to unique title/season with their (already summed) global hours
unique_title_seasons <- latin_table %>%
  group_by(`Show Title`, `Season Title`) %>%
  summarise(
    `Global Hours Viewed` = first(`Global Hours Viewed`),
    .groups = "drop"
  ) %>%
  mutate(`Approx Global Views` = `Global Hours Viewed`) %>%
  arrange(desc(`Global Hours Viewed`))

# Add TOTAL row
total_row <- tibble(
  `Show Title` = "TOTAL",
  `Season Title` = "",
  `Global Hours Viewed` = sum(unique_title_seasons$`Global Hours Viewed`, na.rm = TRUE),
  `Approx Global Views` = sum(unique_title_seasons$`Approx Global Views`, na.rm = TRUE)
)

unique_final <- bind_rows(unique_title_seasons, total_row)

# Render table
datatable(
  unique_final,
  caption = "Unique Titles: Global Hours Viewed and Approximate Global Views (≈1 hr per view)",
  options = list(
    pageLength = 5,
  ...
  formatCurrency(
    c("Global Hours Viewed", "Approx Global Views"),
