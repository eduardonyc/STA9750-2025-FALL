...
cat("India-exclusive NON-ENGLISH, (i.e., implied Hindi content) that achieved global top 10 status:\n")
global_appearing_breakdown %>%
  rename(`Content Type` = content_type, `Number of Shows` = distinct_shows, `Percentage` = percentage) %>%
  datatable(options = list(searching = FALSE, info = FALSE, paging = FALSE))

cat("Total NON-ENGLISH (i.e., implied Hindi content) appearing in global charts:", shows_in_global_count, "of", total_combinations, "\n\n")
...
cat("India-exclusive NON-ENGLISH (i.e., implied Hindi) Content - Yearly Viewing Hours Analysis:\n")
YEARLY_ANALYSIS %>%
  datatable(options = list(searching = FALSE, info = FALSE, paging = FALSE))
