...
SHOW_SUMMARY <- GLOBAL_TOP_10 %>%
  ...
  mutate(
    # Clean up season titles - remove blanks but keep NAs as valid seasons
    season_title = trimws(season_title),
    season_title = na_if(season_title, "")
  ) %>%
  group_by(show_title) %>%
  summarise(
    # Count distinct season titles, treating NA as a valid season
    `Number of Seasons`  = n_distinct(season_title, na.rm = FALSE),
    `Total Hours Viewed` = sum(weekly_hours_viewed, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Join average runtimes
  left_join(RUNTIME_REF, by = "show_title") %>%
  # Calculate approximate viewership
  mutate(
  ...
  # Sort by total hours (convert back to numeric for sorting)
