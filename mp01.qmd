...
# 1) Define Latin America (include Brazil + Mexico + Uruguay, etc.)
latam_countries <- c(
...
# 2) How many LATAM countries charted each show/season?
...
# 3) Global totals across ALL weeks (hours + Netflix weekly_views)
global_totals <- GLOBAL_TOP_10 %>%
  mutate(season_title = coalesce(season_title, "")) %>%
  group_by(show_title, season_title) %>%
  summarise(
    total_hours_viewed = sum(weekly_hours_viewed, na.rm = TRUE),
    total_episode_views = sum(weekly_views, na.rm = TRUE),
    .groups = "drop"
  )

# 4) Merge and order
final <- latam_country_counts %>%
  inner_join(global_totals, by = c("show_title","season_title")) %>%
  arrange(desc(total_hours_viewed))

# 5) Add TOTAL row (note: Countries sums per-title; overlaps are intentionally not de-duplicated)
total_row <- tibble(
  show_title = "TOTAL",
  season_title = "",
  total_hours_viewed = sum(final$total_hours_viewed, na.rm = TRUE),
  total_episode_views = sum(final$total_episode_views, na.rm = TRUE),
  Countries = sum(final$Countries, na.rm = TRUE)
)

final_tbl <- bind_rows(final, total_row) %>%
  transmute(
    `Show Title` = show_title,
    `Season Title` = ifelse(season_title == "", "N/A", season_title),
    `Total Hours Viewed` = total_hours_viewed,
    `Total Episode Views` = total_episode_views,
    Countries
  )

# 6) Render: searchable, 5 per page, with paging controls
datatable(
  final_tbl,
  options = list(
    pageLength = 5,                # show 5 rows per page
    lengthMenu = c(5, 10, 25, 50), # user can switch pages size
  ...
  formatCurrency(
    c("Total Hours Viewed","Total Episode Views"),
