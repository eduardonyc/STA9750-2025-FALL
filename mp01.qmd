...
Netflix's foray into original programming is yielding massive dividends, with unafraid viewers willingly spending their time (and money) with billions of "Strangers" year after year since 2016. For Netflix, this translates into viewership that has surpassed the billion mark. The Stranger Things series has garnered 2.5 billion viewers since its premiere. During its fourth season, it stayed a remarkable 19 weeks in Netflix's Global Top 10 list, with seven of those weeks resting comfortably at number one. Moreover, the series averaged about 13 weeks in the global top 10 across 93 countries, with Pakistan and Ukraine leading the charge at 24 total weeks in the Global top 10.
...
extract_season_num <- function(x) {
  n <- readr::parse_number(dplyr::coalesce(x, ""))
...
# =========================
# 1) Summary artifact for caption
# =========================
...
# =========================
# 2) Build leaderboard (Top N)
#    - Adds Share of Global Peak (n = 50)
#    - Prepare up to Top 20 for exploration
# =========================
ST_TOP_COUNTRIES_WEEKS <- COUNTRY_TOP_10 %>%
  ...
  slice_head(n = 20) %>%  # keep top 20 available
  mutate(
    `Share of Global Peak (n = 50)` = paste0(round((`Weeks in Top 10` / 50) * 100), "%")
  ) %>%
  rename(Country = country_name)

# =========================
# 3) Caption (title + subtitle)
# =========================
cap <- htmltools::tags$caption(
  style = "caption-side: top; text-align:left;",
  htmltools::tags$div(
    style = "font-weight:700; font-size:1.05em; margin-bottom:2px;",
    "Top Countries by Total Weeks in Netflix Top 10 (Stranger Things)"
  ),
  htmltools::tags$div(
    style = "color:#666; font-size:0.9em;",
    htmltools::HTML(
      sprintf(
        "Global reach: <b>%s</b> countries &nbsp;&middot;&nbsp; Avg per-country Top-10 weeks: <b>%s</b> &nbsp;&middot;&nbsp; Peak baseline: <b>n = 50</b>",
        summary_artifact$total_countries,
        summary_artifact$avg_weeks
      )
    )
  )
)
...
# =========================
# 4) Render (Top 5 default; expandable)
# =========================
DT::datatable(
  ST_TOP_COUNTRIES_WEEKS,
  caption = cap,
  escape = FALSE,  # allow HTML in caption
  options = list(
    pageLength = 5,  # show Top 5 initially
    lengthMenu = list(c(5, 10, 15, 20), c('Top 5', 'Top 10', 'Top 15', 'Top 20')),
    searching = TRUE,  # quick look-up
    info = FALSE,
    columnDefs = list(list(className = 'dt-left', targets = "_all"))
  ),
  rownames = FALSE
