...
Following the guidelines for Mini Project #02, the code below contains all the essential content that addresses Tasks 2 through 7. 
...
This analysis integrates data from three federal agencies—the Census Bureau's American Community Survey (ACS), the Census Building Permits database, and the Bureau of Labor Statistics (BLS)—each using distinct identification systems for Core-Based Statistical Areas (CBSAs). 
...
As illustrated in Figure 1, each data source uses a different convention for identifying the same geographic units:
...
This transformation process is essential for the analysis. It ensures geographic accuracy by preventing data mismatches, enables reproducibility by documenting the logic, and demonstrates data quality awareness. The accompanying relationship diagram visually confirms that this complex integration is managed appropriately.
...
**COVID-19 Artifact Note:** 2021’s big jump is a result of 2020’s delayed permits catching up after lockdowns. In other words, this is a data “blip,” not a new trend, seen across several metro areas.
...
# ---- filter to target year (2015 here; change if needed) ----
pop15 <- POPULATION %>%
...
# Calculate last NYC year (retained if needed elsewhere)
...
### Change of Rent Burden Over Time 
...
The **Affordability Gap** metric shows how many percentage points a metro's rent-to-income ratio is above or below the 30% federal affordability standard.
...
This metric is used because it's more intuitive than a raw ratio and directly answers the policy question: "How far from the affordability standard is this city?"
...
This Sunbelt tightening is happening because affordable areas experienced a variety of challenges during the observed period. 

1. Mass Migration: An increase in new residents, many with high-earning potential, migrated to these areas.
2. Slow Building: Existing housing and zoning rules prevented developers from increasing the supply of new housing.
3. Wage-Rent Gap: Rents increased for everyone over this period, but wages have not caught up.

---

#### **Anatomy of a Squeeze: Colorado Springs Rent Burden Evolution**

This metro experienced the largest sustained increase in rent burden, illustrating how affordability can wane over time.
...
To contextualize Colorado Springs, the top end is dominated by Florida. Cape Coral–Fort Myers and Miami both sit around 30.1% RTI—we highlight Miami because its far larger population makes the stakes higher. Several other large Florida metros hover within ~2 percentage points of the 30% line, underscoring how close they are to the affordability edge.

```{r}

# ========================================
# TABLE 2: HIGHEST BURDEN
# ========================================
...
Affordability is strongest in smaller Midwest/Plains metros. Bismarck leads, with Jefferson City, Decatur, Dubuque, and Paducah close behind. Each of these metros are comfortably below the 30% rent standard, signaling real budget relief compared with tighter Sunbelt markets.

```{r}

# ========================================
# TABLE 3: LOWEST BURDEN
# ========================================
...
# ========================================
# STEP 3: CALCULATE 5-YEAR ROLLING POPULATION GROWTH
# ========================================
...
# ========================================
# METRIC 2: GROWTH ACCOMMODATION INDEX
# ========================================
...
We develop two complementary metrics to assess housing supply across U.S. metros areas: current per-capita building rate and responsiveness to population growth.

#### **Instantaneous Housing Supply Index (Metric 1)**

This metric shows how intensively a metro is building right now, focusing on permits issued relative to its current population, regardless of whether the population is rising or falling. It reflects the metro’s near-term commitment to expanding supply.
...
This metric measures whether a metro is permitting enough new homes to keep pace with 5-year population growth.

**Calculation:**

- Permits per growth resident:

$$\text{Permits per Growth} = \frac{\text{Annual Housing Permits}}{\text{5-Year Population Change}}$$
...
**Metric Note:** This metric is not applicable to metros with declining or stable populations (defined as 5-year population change ≤ 100 people). For these metros, Metric 2 is recorded as NA, and their composite score is based solely on Metric 1.

**Data Note:** Population growth includes both net migration and natural increase (births minus deaths). Housing demand is typically higher for growth driven by in-migration, as new households form immediately, compared to natural increase where housing needs develop gradually as children mature.
...
- **Special Case for No Growth**: If 5-year population change ≤ 100 (Metric 2 = NA), the Composite = Metric 1.
...
- Five-year window: Matches <a href="https://www.census.gov/content/dam/Census/library/publications/2018/acs/acs_general_handbook_2018_ch03.pdf" target="_blank">typical housing/ACS 5-year cycles<a/>, smoothing year-to-year noise.
...
sensitivity_2023 %>%
  ...
  opt_row_striping() %>%  # striped rows (hover effect isn't native to gt)
  cols_label(
    Metro = "Metro",
...
Therefore, the **60/40** weight is the best option as it accurately reflects resident composition. Most of the analzyed metros (95%+) consist of existing households needing stable supply, with population growth impacting a smaller share annually. This weighted balance recognizes and awards metros like Punta Gorda that are consistently building for *all* residents. 
...
- **Criteria 1 (C3)**: (positive 5-year change, 2018–2023), and  
...
# Filter to only 3/4 and 4/4 metros for cleaner visualization
plot_data <- combined_analysis %>%
  ...
  mutate(
    ...
    hover_text = paste0(
      "<b>", metro_clean, "</b><br>",
      "Population Growth: ", comma(abs(pop_change_5yr))
    )
  )
...
p <- ggplot(plot_data, aes(
  ...
  # Points with color and size
  geom_point(
    aes(color = factor(yimby_score), 
  ...
  scale_color_manual(
    ...
    name = "YIMBY Score",
    labels = c("3" = "3/4 (Strong)", "4" = "4/4 (Complete)")
  ) +
  
  # Size scale (won't show in legend, but controls visual size)
  scale_size_continuous(
    range = c(3, 16),
    guide = "none"  # Accept it won't show, info is in hover
  ) +
...
Some metros became more affordable without building new homes, but this wasn't a result of luck. It occured as a result of the following: 

- **People Migrated**: Most of these metros (e.g., Rust Belt metros) lost population, reducing demand for housing.
- **Wages Increased**: A few metros had a sudden industry surge (like energy), raising salaries faster than rent.
...
Across the sample, 12 metros meeting all 4 criteria cut rent burden by ~1.4 pp between 2014–2016 and 2021–2023 despite population growth. As a result, there is clear, outcome-level evidence that sustained building can relieve pressure.

**The Data Story**

- **Where it works**: Success clusters in the Southeast/Sunbelt and a few high-growth markets—places with faster permitting and fewer density constraints.
...
A future policy implication is clear: regulatory reform that enables sustained construction may lead to improved affordability within a 7-9 year timeframe.

## Task 7: Policy Brief - Sponsor City Selection

### Primary Sponsor: YIMBY Success City

Building on our Task 6 analysis, we identified **Punta Gorda, FL** as the flagship YIMBY success story. It meets all four YIMBY tests: high early burden, burden fell (-0.3 pp), strong growth (+13.2%), and nation-leading building (housing index ~292). A key takeaway is that when metros build at scale, they can grow and keep rents in check.

---

### Co-Sponsor: Opportunity City Selection

To identify optimal co-sponsor cities, we refined our analysis to target metros exhibiting the inverse pattern, specifically fast-growing metros where affordability is slipping because construction lags. Our three-part test is simple:

- Population growth (real demand),
- Burden flat/worse (no relief),
- Below-average building (under-supply).

Metros that satisfy this test are on a potential path to crisis, making them the best candidates for a federal YIMBY incentive to enhance supply and shift rents down.
...
**Recommended Co-Sponsor:** **Atlanta, GA Metro** — With 6.3M residents and 12.4% population growth, this metro exemplifies a booming metro where rent burden worsened by 3.0 pp despite demand, due to below-average housing construction (score: 84). This is exactly the profile that a policy bill would target: a metro candidate eligible for federal incentives to unlock housing supply to reduce rents.
...
# ---- render with gt (full-width note + styling) ----
gt_tbl <- tbl %>%
  gt() %>%
  # Use tab_options and tab_header like your other tables
  tab_options(
    table.width = pct(100),
    table.align = "left",
    heading.align = "left",
...
The coalition centers on 72, Accommodations & Food Services, and 44, Retail Trade. These represent large worker bases having clear affordability gains from YIMBY policy, and strong, organized industry partners.
...
Atlanta exemplifies the supply-constrained growth pattern federal YIMBY grants target: 6.3 million residents growing 12.4% over five years, yet rent burden worsened by 3.0 percentage points—the most among large metros—due to below-average housing construction (score: 83.5). This gap leaves 1.2 million service workers paying 4-10 percentage points more of wages to rent than workers in successful metros.

---

## Unlocking Atlanta Metro's Housing Supply and Job Growth

The Atlanta metro currently employs 331,350 construction workers. If the metro matched Punta Gorda's proven construction intensity of 102.9 workers per 1,000 residents, it would sustain 648,834 construction jobs—a potential increase of 317,484 jobs representing 96% sector growth. More construction workers can lead to more housing built, placing more housing in the supply to reduce pressure on rents.
...
# ===== Silent data extraction (no output) =====
...
* Construction ~17% below national average (Housing Growth Index: 83.5)
* Despite 12.4% population growth, rent burden worsened by 3.0pp
* Federal support can create **317,484 construction jobs** (96% increase from current 331K)
...
* **For Construction Trades** (AFL-CIO Building Trades, National Association of Home Builders): Atlanta matching Punta Gorda's building rate creates **317,484 additional jobs**, higher-paying, non-outsourceable employment that supports local trade unions and contractors.

* **For Service Workers** (UNITE HERE, United Food and Commercial Workers): Reduces the 9.6pp rent-burden gap for Hospitality workers (**$2,619 savings**) and 4.0pp for Retail workers (**$1,976 savings**). **Scale:** 1.2M workers in Atlanta, 38K in Punta Gorda (BLS QCEW 2023). **Total wage power restored: ~$3 billion.**
...
This bill creates 300K+ jobs, saves workers $2,400 annually, and restores $3B in wages, resulting in a pro-worker, pro-business solution with no mandates.
...
* **Housing Growth Index:** Are new housing permits keeping pace with metro size and 5-year population growth? (Higher = building enough to meet demand.)
...
* **Accountability:** Public dashboards track results; independent audits verify spending; five-year sunset forces evaluation before renewal, with expectations in a rise in building permits (years 2-3), increases in local construction jobs (years 3-4), and measurable relief from high rents (years 4-5). 
...
* **Administration:** Uses existing HUD competitive grant processes; no new bureaucracy required.
...
**We urge you to co-sponsor the Federal YIMBY Housing Growth Incentive Bill.**