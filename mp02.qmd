...
The second transformation links Building Permits to BLS wage data by formatting the integer CBSA code into the BLS-specific FIPS string format.
...
- **BLS Wage Data** use formatted `FIPS` strings with "C" prefix and that need zero-padding (e.g., "C1018" to normalize to "10180")
...
- Mass Migration: An increase in new residents, many with high-earning potential, migrated to these areas.
- Slow Building: Existing housing and zoning rules prevented developers from increasing the supply of new housing.
- Wage-Rent Gap: Rents increased for everyone over this period, but wages have not caught up.
...
Colorado Springs is on a slippery path. While it's still more affordable than metros on the edge of an affordability gap, like Florida (as shown in Table 3), it's catching up fast. It has already lost 5.6 points of affordability since 2009. If this trend continues, it will become another unaffordable metro within the next two decades.

This shows that rapid growth without enough building can lead to an affordability crisis. 
...
**Data Note:** Population growth includes both net migration and natural increase (births minus deaths). Housing demand is typically higher for growth driven by inward migration, as new households form immediately, compared to natural increase where housing needs develop gradually as families expand.
...
Some metros became more affordable without building new homes, but this wasn't circumstance. It occured as a result of the following: 

- **People Migrated**: Most of these metros (e.g., Rust Belt metros) lost population, reducing demand for housing.
- **Wages Increased**: A few metros had sudden industry surges (like energy), raising salaries faster than rent.
...
Atlanta exemplifies the supply-constrained growth pattern federal YIMBY grants target: 6.3 million residents growing 12.4% over five years, yet rent burden worsened by 3.0 percentage points—the most among large metros—due to below-average housing construction (score: 84). This gap leaves 1.2 million service workers paying 4-10 percentage points more of wages to rent than workers in successful metros.

---

## Unlocking Atlanta Metro's Housing Supply and Job Growth

The Atlanta metro currently employs 331,350 construction workers. If the metro matched Punta Gorda's proven construction intensity of 102.9 workers per 1,000 residents, it would sustain 648,834 construction jobs, representing a potential increase of 317,484 jobs (or 96% sector growth). More construction workers can lead to more housing built, placing more housing in the supply to reduce pressure on rents.
...
- **For Service Workers** (UNITE HERE, United Food and Commercial Workers): Reduces the 9.6pp rent-burden gap for Hospitality workers (**$2,619 savings**) and 4.0pp for Retail workers (**$1,976 savings**). **Scale:** 1.2 million workers in Atlanta, 38K in Punta Gorda (BLS QCEW 2023). **Total wage power restored: ~$3 billion.**
