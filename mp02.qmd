...
These datasets integrate into all subsequent tasks to analyze housing affordability across metropolitan areas in the United States.
...
top_10_states <- state_income %>%
  ...
  mutate(
    ...
    note = case_when(
      ...
      state %in% c("NY", "MD", "VA", "PA", "NJ") ~ "⚠️ Contains Multi-State CBSAs",
      TRUE ~ ""
    )
  ) %>%
...
datatable(
  ...
  caption = tags$caption(
    ...
    HTML(paste0("Table 3: Top 5 Highest Rent Burden Metro Areas — ", latest_year, 
        ...
        "Note: Miami and Cape Coral tie at 30.1% RTI. Miami is highlighted as the larger metro (6.1M residents vs. 830K).</span>"))
  )
) %>%
...
Therefore, the **60/40** weight is the best option as it accurately reflects resident composition. Most of the analyzed metros consist of existing households needing stable supply, with population growth impacting a smaller share annually. This weighted balance recognizes and awards metros like Punta Gorda that are consistently building for *all* residents.
...
p <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Policy Brief sponsors: Punta Gorda, FL (Success) at x=291.7, y=-0.3 | Atlanta, GA (Opportunity) at x=84, y=+3.0",
...
**Looking Ahead: Connection to Policy Brief (Task 7):** The two sponsor cities for the proposed Federal YIMBY Housing Growth Incentive Bill are visible in this plot:

- **Punta Gorda, FL** (Success Sponsor): Located at x≈292, y≈-0.3 in the success quadrant (Q4)
- **Atlanta, GA** (Opportunity Sponsor): Located at x≈84, y≈+3.0 in the challenging quadrant (Q1)

These positions demonstrate why they were selected: Punta Gorda shows proven success in building and maintaining affordability, while Atlanta shows an urgent need for federal housing support.
