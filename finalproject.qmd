---
title: "STA 9750 Final Project - A Market Realigned: How COVID-19 Altered the Value of Educational Attainment"
...
The COVID-19 pandemic introduced a significant structural break in urban living patterns. Unprecedented disruptions altered individual needs and preferences, challenging the neighborhood characteristics that traditionally predicted property value premiums. These shifts drive our research team’s investigation into how property value drivers responded to these new dynamics, guided by the Overarching Question (OQ):
...
While the <a href="https://tiffany-ngli.github.io/STA9750-2025-FALL/Summary%20Report.html" target="_blank">team's broader effort</a> examines crime, density, job accessibility, and transit, this analysis isolates Educational Attainment (EA). Historically, High-EA neighborhoods commanded significant price premiums from well-known <a href="https://www.annualreviews.org/deliver/fulltext/resource/11/1/annurev-resource-100518-094151.pdf?itemId=/content/journals/10.1146/annurev-resource-100518-094151&mimeType=application/pdf" target="_blank">agglomeration effects</a>. However, the pandemic’s disruption of physical proximity necessitates a re-evaluation of this premium. Consequently, this study seeks to answer the specific question (SQ):

*Did the strength of the relationship between neighborhood educational attainment and property values change post-COVID, and did this change differ across NYC's CDs?*

### Hypotheses

Given the pandemic's impact on work-life demands and dwelling needs, this question explores two hypotheses.

**Hypothesis 1**: The positive correlation between EA and property values would strengthen post-COVID.

**Hypothesis 2**: High-EA CDs would experience stronger property value growth post-COVID as remote work freed professionals to prioritize neighborhood amenities over commute times.
...
This analysis integrates four data sources through programmatic retrieval, resolving incongruent spatial boundaries to align coding systems across federal and city databases.

### Setup and Configuration

Loading required packages and defining global constants enables the programmatic assembly of a 59 CD-level panel for 2017–2019 vs. 2021–2023 and merging it with baseline 2019 American Community Survey (ACS) education variables.
...
library(plotly)  # spatial heterogeneity analysis   
...
The `get_nyc_cd()` function retrieves the official shapefile from the NYC Department of City Planning, unzips it, and constructs standardized identifiers for all 59 CDs. 
...
The `get_pluto_cd_crosswalk()` function uses raw data from NY Open Data to implement a Borough–Block–Lot (BBL) to CD crosswalk. This process normalizes approximately 860,000 BBLs, resolving formatting inconsistencies and creating standardized linkages between individual tax lots and CDs. 
...
The `get_dof_sales_year_boro()` function automates the collection and cleaning of Annualized Sales reports from the NYC Department of Finance (DOF). Additionally, exclusion criteria filter transactions under $10,000 and restrict the sample to <a href="https://www.nyc.gov/site/finance/property/definitions-of-property-assessment-terms.page" target="_blank">residential tax classes</a>, specifically 1, 2, 2A, 2B, and 2C.  
...
A two-stage strategy combines exact BBL matching with a block-level fallback. While exact matching loses approximately 25% of transactions (e.g., condo billing BBLs), the fallback links these records to their specific city block, achieving a 100% match rate, as shown in Table 2. 
...
build_cd_sales_panel <- function(
  ...
  # 8. Final summary (used in previous renderings for confirmation)  
  message("")
...
Figure 1 highlights this consistency across boroughs, minimizing selection bias by recovering high-density transactions that standard matching would miss.
...
Treating EA as a fixed baseline is a critical research control. This step is necessary to isolate pure market demand from potential extraneous variables. Updating education data post-COVID would obfuscate the source of price changes (e.g., preference changes or population shifts). Consequently, holding education constant at 2019 levels  ensures that results yield an accurate measure of changing housing demand. 

Table 3 below demonstrates this fixed baseline strategy, showing that the same 2019 ACS education data is consistently applied across all six research years (2017-2023), enabling appropriate isolation of pandemic-era market shifts.
...
```
<div style="height: 15px;"></div>
Omitting 2020 is essential to ensure analysis integrity. Given the acute disruptions from this year, any statistical anomalies may distort long-term trend analysis; thus, bypassing it yields a clearer view of the market's post-COVID response. 

The final data integration shown in Table 5 confirms critical merging (e.g., median prices by CD-year) with education baselines held constant at 2019 levels. 
...
The approaches highlighted in the <a href="https://eduardonyc.github.io/STA9750-2025-FALL/finalproject.html#data-acquisition-and-processing" target="_blank">Data Acquisition and Processing</a> section create a balanced panel of 59 CDs encompassing over six years ($N = 354$) worth of data. This longitudinal structure supports this research's <a href="https://www.publichealth.columbia.edu/research/population-health-methods/difference-difference-estimation" target="_blank">Difference-in-Differences</a> (DiD) analysis, ensuring that any identified trends accurately link to post-pandemic shifts.
...
Table 6 shows clear distinctions between strata: Low-EA CDs yield a 19% BA+ Attainment (Bachelor’s degree or higher) average, while High-EA CDs average 53%. This variation establishes a tangible baseline for comparing how housing markets evolved.
...
A DiD design is a favorable approach to filter out factors (e.g., economic shifts and neighborhood characteristics) impacting trends, allowing for a strict focus on the pandemic's effect. This research tests the <a href="https://theeffectbook.net/ch-DifferenceinDifference.html#untreated-groups-and-parallel-trends" target="_blank">Parallel-Trends Assumption (PTA)</a> to support a DiD interpretation of genuine structural shift in market response rather than pre-existing trends.

#### **Logarithmic Transformation**

This analysis implements a logarithmic transformation of property values to meaningfully draw comparisons. Since High-EA CDs begin at significantly higher baseline values, standardization yields relative appreciation rates, ensuring comparability across terciles. For example, a 0.10 log point shift results in an approximate 10% change in value.

Figure 2 shows how High-EA CDs start at roughly twice the price level of Low-EA CDs. However, the three trajectories rise at similar rates, reinforcing the PTA.
...
#### **Education and Value: A Lesson on NYC Geography** 
...
kable(
  top_bottom_tbl,
  caption = "**Table 9: CDs with Highest and Lowest Educational Attainment (2019)**",
...
```
<div style="height: 15px;"></div>
The Leaflet map below highlights EA disparities across CDs. Clicking on an individual CD reveals its percentage of EA.
...
kable(
  regression_table,
  caption = "**Table 10: Pre-COVID Baseline Regression - Median Sale Price on EA**",
  align = c("l", rep("l", 6))
)
...
The regression line's flatter slope indicates that each additional pp of BA+ Attainment predicts a smaller price difference than in the pre-COVID period. Table 12 provides additional clarity with actual price changes across terciles, revealing which groups appreciated fastest.
...
```
<div style="height: 15px;"></div>
Contextualizing this reversal shows that Low-EA CDs gained nearly $29,000 more than High-EA CDs on average. In other words, a home purchased in a traditionally "less desirable" neighborhood in 2017 significantly outperformed its High-EA counterpart by 2023.
...
```
<div style="height: 15px;"></div>
Differences between high and low terciles is statistically significant at *p* = 0.002, with a 95% confidence interval excluding zero. As a result, this outcome indicates that the pattern is unlikely to have occurred by chance. 
...
Non-overlapping confidence intervals confirm distinct economic outcomes between the High-EA and Low-EA groups, indicating the 14.2 pp spread represents structural shifts rather than an anomaly.
...
As shown in Table 14, each additional pp of BA+ Attainment predicts 0.380 pp less price growth. This statistically significant (*p* < 0.001) negative coefficient directly contradicts the pre-COVID pattern, where High-EA predicted higher prices. The relationship has not only weakened, but it has also reversed.
...
Table 14 predicts a -0.380 pp change in growth per additional BA+ Attainment point. Given the roughly 40 pp gap between High and Low terciles (Table 6), the regression model predicts High-EA CDs should grow 15.3 pp less than Low-EA CDs.
...
```
<div style="height: 15px;"></div>
The regression-based prediction (-15.3 pp, from High's perspective) closely matches the observed tercile difference (+14.2 pp, from Low's perspective). This minimal deviation validates the consistency of the results, regardless of using a tercile or regression approach.

Whether comparing discrete education tiers or modeling continuous relationships, the conclusion remains the same: Low-EA CDs experienced substantially faster price growth during the post-COVID period, with the magnitude of this reversal measuring approximately 14 to 15 pp.

## Spatial Variation: Examining Consistency Across Boroughs
...
```
<div style="height: 15px;"></div>
Manhattan exhibited the highest volatility ($SD = 15.9\%$). Queens showed similar variation ($SD=11.7\%$), while Brooklyn remained relatively more stable ($SD=8.6\%$).
...
Brooklyn's near-zero slope (-0.009) and weak correlation (*r* = -0.005) suggests that other factors (e.g., <a href="https://nextcity.org/urbanist-news/in-brooklyn-americas-gentrification-epicenter-building-on-a-model-for-commu" target="_blank">gentrification</a>) drove appreciation more than EA. Additionally, its close proximity to Manhattan likely outweighed EA in determining appreciation. Consequently, Brooklyn's unique outcome calls for deeper investigation in future research.
...
This figure further confirms that no borough experienced the traditional positive education-growth relationship during the post-COVID period. In four of five boroughs, there was a weak-to-strong negative relationship, with slopes ranging from -0.230 to -0.682.  
...
p <- p |>
  layout(
    title = list(
      text = paste0(
        "<b style='font-size:16px'>Figure 7: Borough-Level Education-Growth Relationship</b>",
        "<br>",
    ...
    xaxis = list(
      title = list(
        text = "Educational Attainment (% BA+, 2019)",
        standoff = 25   
      ),
      ticksuffix = "%",
      gridcolor = "#E5E5E5",
...
Remote work <a href="https://www.cnn.com/2023/09/03/homes/remote-work-housing" target="_blank">reduced the need for living near employment-rich and amenity-filled hubs</a>, usually concentrated in High-EA CDs. Also, affordability pressures led buyers to migrate toward <a href="https://www.freddiemac.com/research/pdf/202206-Note-Migration-08.pdf" target="_blank">undervalued CDs in the outer-borough areas</a>, offering greater access to homeownership opportunities. Lastly, lower valuation CDs experienced post-pandemic market correction, leading to catch-up growth, or <a href="https://siepr.stanford.edu/publications/policy-brief/donut-effect-how-covid-19-shapes-real-estate" target="_blank">a donut effect</a>, shifting market demand and creating higher prices in once affordable CDs. 

These factors likely created a compounding market cycle. As the premium for High-EA CDs diminished, demand shifted toward outer-borough markets, accelerating appreciation in Low-EA CDs while High-EA growth stagnated. However, this represents a realignment of growth dynamics rather than market structure reversal High-EA neighborhoods maintained their pricing premiums but lost their advantage in appreciation rates.
...
- Provides baseline context for <a href="https://kelmli.github.io/STA9750-2025-FALL/final_proj.html" target="_blank">crime results</a>: High-crime CDs experienced the largest crime increases (+1.58 per 1,000), but crime showed no relationship with property values in either individual ($R^2$ ≈ 0.000) or the team model (*β* = -0.029, *p* = 0.765), confirming crime and property values respond independently to neighborhood characteristics.

In the team model, EA emerged as the sole statistically significant predictor (*β* = -0.402, *p* < 0.001). Although the full model improved explanatory power ($R^2 = 0.370$) over the education-only model ($R^2 = 0.291$), the lack of significance for density, crime, and transit confirms that EA was the primary characteristic driving appreciation variance. Consequently, this reversal represents the dominant structural shift of the period, suggesting that the pandemic fundamentally changed NYC's urban economic geography.

## Limitations and Conclusion

While this analysis provides clear evidence of a post-pandemic reversal, several limitations arise from the results. First, CD level data may obscure location-specific variations, such as gentrification within Low-EA areas. Second, residents in High-EA CDs may have held onto their properties; thus, low growth may have resulted from a lack of available real estate. Finally, as the data extends only through 2023, it is unclear if these findings are indicative of temporary disruptions or longstanding, permanent shifts, considering recent employer mandates calling for <a href="https://www.usatoday.com/story/money/2025/08/27/work-from-home-workers-defy-rto-mandates/85821219007/" target="_blank">employees to return to physical work locations</a>. 

Despite these limitations, this research reveals pre- and post-COVID shifts reversed the relationship between neighborhood EA and property values in NYC. The pandemic revised buyer preferences, disrupting traditional drivers of value. Consequently, affordability pressures created a new urban landscape where traditional High-EA CDs are less significant, driven by demand shifts toward outer-borough value.
...
- NYC Comptroller. *New York: A City of Diverse Neighborhoods* (report link as cited in document).  
  <https://comptroller.nyc.gov/reports/new-york-a-city-of-diverse-evolving-neighborhoods/>
...
- Alarcon, E., Brinson, M., Li, K., Ng Li, T., & Socoy, J. (2025). *STA 9750 Team Summary Report: COVID-19 and NYC Property Values.* Baruch College.  
  <https://tiffany-ngli.github.io/STA9750-2025-FALL/Summary%20Report.html>
