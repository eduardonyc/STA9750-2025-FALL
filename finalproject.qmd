...
Figure 1 demonstrates the two-stage matching approach's effectiveness across all year-borough combinations. The consistent match rates allow for unbiased geographic analysis by recovering transactions (e.g., condos) that exact BBL matching alone would miss.
...
Table 6 shows clear distinctions between strata, with Low-education CDs yielding a 19% BA+ Attainment (Bachelor's degree or higher) average, while High-education CDs average 53% BA+ Attainment. This variation establishes a tangible baseline for comparing how CD housing markets evolved during the pandemic era.
...
Figure 4 shows how High-EA CDs start at roughly twice the price level of low-education districts. However, the three trajectories rise at similar rates, reinforcing the PTA.
...
ggplot(
  ...
  labs(
    title    = "Figure 4: Pre-COVID Price Trends by Education Level (2017–2019)",
...
Pre-COVID, there was a strong linear correlation (*r* = 0.78) between neighborhood EA and its property value, as shown in Figure 2 below. 
...
$$\widehat{\text{Median Price}}_{\text{Pre-COVID}} = \$235{,}462 + \$13{,}842 \times \text{BA+\%}$$
...
The post-COVID scatter plot below reveals a weaker but still positive relationship between EA and property values (*r* = 0.74, down from *r* = 0.78 pre-COVID). 
...
These findings indicate a remarkable reversal of the traditional education premium. From 2017-19 to 2021-23, Low-EA CDs experienced 26.03% median price growth, whereas Medium-EA CDs and High-education CDs grew only by 15.03% and 11.86%, respectively. The most notable finding is that High-EA CDs grew at less than half the rate of its Low-EA counterparts, representing a 14.2% point difference.
...
Figure 6 below shows the profound magnitude of this reversal. 
...
ggplot(
  ...
  labs(
    title    = "Figure 6: Property Value Growth by Education Tercile",
...
$$\widehat{\text{Median Price}} = \$235,462 + \$13,842 \times \text{BA+\%}$$ 
...
From Table 14, each additional pp increase in BA+ Attainment predicts a -0.380 pp change in price growth. Moreover, Table 6 highlighted the average education gap between High and Low terciles to be roughly 40 pp. 
...
This means the regression model predicts that High-EA CDs should grow 15.3 pp less than Low-EA CDs.
...
Brooklyn's near-zero slope (-0.005) and weak correlation (*r* = -0.008) suggests that other factors (e.g., <a href="https://nextcity.org/urbanist-news/in-brooklyn-americas-gentrification-epicenter-building-on-a-model-for-commu" target="_blank">gentrification</a>) drove appreciation more than EA. Additionally, its close proximity to Manhattan may have likely outweighed EA in determining appreciation. Consequently, Brooklyn's unique outcome calls for deeper investigation in future research.
