...
This approach creates a balanced panel of 354 CD-year observations (59 CDs over 6 years), enabling thorough Difference-in-Differences (DiD) in this analysis using consistent neighborhood tracking across time periods.
...
The `get_pluto_cd_crosswalk()` function uses raw data from NYC Open Data, normalizing approximately 870,000 Borough-Block-Lot (BBL) codes. This process resolves formatting inconsistencies to standardized linkages between individual tax lots and CDs.
...
This process removed 1,154 invalid entries (0.1%), preventing silent join failures downstream.
...
tibble(
  ...
  kable(caption = "PLUTO BBL Normalization Results")
...
tibble(
  ...
  kable(
    caption = "Enhanced BBL Matching Results: Two-Stage Approach",
    align = c("l", "r", "r")
  )
...
Treating EA as a fixed baseline is a critical research control. This step is necessary to isolate pure market demand from potential lurking variables. Updating this data post-COVID would also obfuscate the source of price changes (e.g., preference changes or population shifts). Consequently, holding education constant ensures that results yield an accurate measure of changing housing demand.

```{r}

#| label: sanity-check-period-coverage
#| include: false
#| message: false
#| warning: false

# Sales panel coverage by period and year  

sales_cov <- cd_sales_panel |>
  count(period, year) |>
  mutate(period = clean_period(period)) |>
  arrange(period, year)

kable(
sales_cov,
caption   = "Coverage of sales panel by period and year",
col.names = c("Period", "Year", "CD–Years")
)

# ACS education coverage by period and ACS end year  

edu_cov <- cd_education_panel |>
  count(period, acs_year) |>
  mutate(period = clean_period(period)) |>
  arrange(period, acs_year)


kable(
edu_cov,
caption   = "Coverage of ACS education panel by period and ACS end year",
col.names = c("Period", "ACS End Year", "CDs")
)

# Structural checks (silent)  
...
This analysis compares two three-year periods: pre-COVID (2017-2019) and post-COVID (2021-2023). 
...
The final data integration merges sales data (e.g., median prices by CD-year) with education baselines held constant at 2019 levels. Consequently, this approach creates a balanced panel of 354 CD-year observations, with 59 CDs encompassing six years worth of data. This structure facilitates a <a href="https://www.publichealth.columbia.edu/research/population-health-methods/difference-difference-estimation" target="_blank">Difference-in-Differences</a> (DiD) analysis,  ensuring that any identified trends accurately link to post-pandemic shifts.
...
kable(
  temporal_scope,
  caption = "**Temporal Scope of Analysis**",
...
kable(
  edu_checks_summary,
  caption = "**Merged Panel Diagnostics** (all values should be constant within CD–period)",
  col.names = c(
    "Min Years", "Max Years",
...
Table 1 shows clear delineation, with Low-education CDs yielding a 19% BA+ Attainment average, while High-education CDs average 53% BA+ Attainment. This variation establishes a tangible baseline for comparing how CD housing markets evolved during the pandemic era.
...
kable(
  tercile_summary,
  caption = "**Table 1. Education Tercile Ranges**",
  col.names = c(
    "Education Group",
    "Number of CDs",
    "Min BA+ (%)",
...
#| label: pretrend-diagnostic
#| echo: false
#| message: false
#| warning: false
#| fig-width: 8
#| fig-height: 5
#| fig-cap: "**Figure 1. Pre-COVID Price Trends by Education Level (2017–2019).** Average log median sale price trajectories were roughly parallel across education terciles, supporting the parallel trends assumption underlying the difference-in-differences design."
...
ggplot(
  ...
  labs(
    title    = "Pre-COVID Price Trends by Education Level (2017–2019)",
...
Table 2 shows stable growth consistency, with annual growth ranging from 3.0% to 8.2%.
...
kable(
  pretrend_slopes,
  caption = "**Table 2. Pre-COVID Annual Price Growth by Education Group**",
  col.names = c(
    "Education Group",
    "Log-Point Slope",
    "Annual % Growth"
